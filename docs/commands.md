...
```bash
coolhand [--client-id ID] proxy-wrap [--] <command> [args...]
```
...
```bash
coolhand proxy-wrap -- kimi                    # starts kimi with capture on
coolhand proxy-wrap kimi --resume              # the -- is optional
coolhand --client-id acme proxy-wrap -- kimi   # capture under the "acme" account
```
...
| Flag | Description |
|------|-------------|
| `--client-id ID` | Use a specific stored client (also `COOLHAND_CLIENT_ID` env var). Must come **before** `proxy-wrap` — anything after it is forwarded to the wrapped command verbatim. |
