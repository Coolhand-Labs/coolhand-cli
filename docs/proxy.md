...
**Batch-prediction-job status polling is excluded from capture,** matching coolhand-node's own exclude list, so those requests aren't uploaded as if they were inference calls. Other non-inference/telemetry traffic to matched hostnames (e.g. Claude Code's own event-logging and directory/environment-poll requests) is not yet filtered — this is a data-minimization/noise issue rather than a security concern, and is tracked as a follow-up pending an upstream `coolhand-node` change.
